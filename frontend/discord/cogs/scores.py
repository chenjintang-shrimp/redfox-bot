...
class Scores(Cog):
    ...
    # @commands.hybrid_command(
    #     name="t", description="Query your best scores (Today's BP)"
    # )
    # @app_commands.describe(user="osu! username or mention")
    ...
    #     # type=best
    #     # Note: Filtering 24h refreshed BP is done in renderer if implemented,
    #     # or simplified to just showing BP list for now as per "not urgent" feedback
    #     content = await render_user_score_list(user_id, "best", include_fails=False, page=1)
    #     total_pages = await get_user_scores_page_count(user_id, "best", include_fails=False)

    #     if total_pages > 1:
    #         view = UserScoresPaginationView(user_id, "best", False, ctx.author.id, total_pages)
    #         await ctx.send(content=content, view=view)
    #     else:
    #         await ctx.send(content=content)
